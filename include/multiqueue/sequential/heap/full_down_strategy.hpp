...
struct FullDown {
    ...
    static typename Heap::size_type sift_up_hole(Heap &heap, typename Heap::size_type index,
                                                 typename Heap::value_type const &v) {
        assert(index < heap.size());
        typename Heap::size_type parent;
        while (index > 0 && (parent = heap.parent_index(index), heap.compare(heap.c[parent], v))) {
            heap.c[index] = std::move(heap.c[parent]);
            index = parent;
        }
        return index;
    }
    ...
    static typename Heap::size_type remove(Heap &heap, typename Heap::size_type index) {
        ...
        while (index < last_parent) {
            ...
            heap.c[index] = std::move(heap.c[child]);
            index = child;
        }
        ...
        return sift_up_hole(heap, index, heap.c.back());
    }
};
