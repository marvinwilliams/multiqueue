...
class NativeBinaryHeap {
 ...
 private:
  ...
  void constexpr sift_down(size_t pos) noexcept {
    ...
    while (pos < first_incomplete_node) {
      auto const min_child =
          pos * 2 + (comp_(data_[pos * 2 + 1], data_[pos * 2 + 2]) ? 1 : 2);
    ...
    if (((len & 1) == 0 && pos == first_incomplete_node) ||
        ((len & 1) != 0 && pos + 2 == len)) {
 ...
 public:
  ...
  constexpr const_iterator cbegin() noexcept { return data_.cbegin(); }

  constexpr const_iterator cend() noexcept { return data_.cend(); }

  constexpr size_t size() const noexcept { return data_.size(); }

  constexpr size_t max_size() const noexcept { return data_.max_size(); }

  constexpr bool empty() const noexcept { return data_.empty(); }

  constexpr const_reference top() const { return data_.front(); }

  constexpr void reserve() { data_.reserve(); }
...
inline void swap(NativeBinaryHeap<T, Comparator, Container>& lhs,
                 NativeBinaryHeap<T, Comparator, Container>&
