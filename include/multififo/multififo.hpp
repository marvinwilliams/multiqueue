...
template <typename T, typename SelectionStrategy = MULTIFIFO_DEFAULT_SELECTION_STRATEGY,
...
class Multififo {
   ...
   public:
    ...
    using param_type = Multififo<SelectionStrategy>;
   ...
   public:
    class alignas(2 * L1_CACHE_LINESIZE) Handle {
       ...
       private:
        explicit Handle(Multififo &mf, unsigned int id, std::uint64_t seed) noexcept : mf_{mf}, data_{seed, id} {
        }

       public:
        bool try_extract_top(reference retval) noexcept {
            {
                auto const [first, second] = mf_.selector_.get_delete_indices(data_);
                auto const [queue, tick] =
                    mf_.select_delete_queue(mf_.queue_list_.get() + first, mf_.queue_list_.get() + second);
                if (!queue) {
                    // Both pqs are empty
                    mf_.selector_.delete_index_used(true, data_);
                    return false;
                }
                if (queue->try_lock_if_key(tick)) {
            ...
            do {
                auto const [first, second] = mf_.selector_.get_fallback_delete_indices(data_);
                auto const [queue, key] =
                    mf_.select_delete_queue(mf_.queue_list_.get() + first, mf_.queue_list_.get() + second);
                if (!queue) {
                    // Both pqs are empty
        ...
        void push(const_reference value) noexcept {
            {
                auto const queue = mf_.queue_list_.get() + mf_.selector_.get_push_index(data_);
                if (queue->try_lock()) {
                    queue->push(value);
                    queue->unlock();
            ...
            do {
                auto const queue = mf_.queue_list_.get() + mf_.selector_.get_fallback_push_index(data_);
                if (queue->try_lock()) {
                    queue->push(value);
                    queue->unlock();
   ...
   private:
    using queue_alloc_type = typename std::allocator_traits<allocator_type>::template rebind_alloc<guarded_fifo_type>;
    ...
    struct QueueDeleter {
        ...
        void operator()(guarded_fifo_type *queue_list) noexcept {
            for (guarded_fifo_type *s = queue_list; s != queue_list + mf.num_queues_; ++s) {
   ...
   private:
    ...
    void abort_on_data_misalignment() {
        for (guarded_fifo_type *s = queue_list_.get(); s != queue_list_.get() + num_queues(); ++s) {
    ...
    std::pair<guarded_fifo_type *, key_type> select_delete_queue(guarded_fifo_type *first,
                                                                 guarded_fifo_type *second) noexcept {
        auto const first_tick = first->get_tick();
        auto const second_tick = second->get_tick();
        if (first_tick == get_sentinel() && second_tick == get_sentinel()) {
            return {nullptr, get_sentinel()};
        }
        if (second_key == get_sentinel() || first_tick < second_tick) {
            return {first, first_tick};
        } else {
            return {second, second_tick};
        }
    }
   ...
   public:
    explicit Multififo(unsigned int num_threads, param_type const &params,
        ...
        assert(params.c > 0);
        queue_list_.reset(queue_alloc_traits::allocate(alloc_, num_queues_));  // Empty unique_ptr does not call deleter
        for (guarded_fifo_type *queue = queue_list_.get(); queue != queue_list_.get() + num_queues_; ++queue) {
    ...
    explicit Multififo(size_type /* initial_capacity */, unsigned int num_threads, param_type const &params,
                       allocator_type const &alloc = allocator_type())
        : queue_list_(nullptr, QueueDeleter(*this)),
          num_queues_{num_threads * params.c},
          alloc_{alloc},
    ...
    static constexpr key_type get_sentinel() noexcept {
        return guarded_fifo_type::get_sentinel();
    }

    constexpr size_type num_queues() const noexcept {
        return num_queues_;
    }
