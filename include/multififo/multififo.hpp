...
class Multififo {
   ...
   public:
    class alignas(2 * L1_CACHE_LINESIZE) Handle {
       ...
       private:
        explicit Handle(Multififo &mf, unsigned int id,
       ...
       public:
        bool try_extract_top(reference retval) noexcept {
            ...
            if (first_tick == tick_type::max() &&
                second_tick == tick_type::max()) {
            ...
            if (mf_.queue_list_[indices.first].try_lock_if_earlier(
                    second_tick)) {
            ...
            } while (!mf_.queue_list_[indices.first].try_lock_if_earlier(
                second_tick));
            mf_.queue_list_[indices.first].extract_top(retval);
            mf_.queue_list_[indices.first].unlock();
            mf_.selector_.delete_index_used(false, data_);
        ...
        bool is_empty(size_type pos) noexcept {
            assert(pos < mf_.num_queues_);
            return mf_.queue_list_[pos].empty();
        }
    };
   ...
   private:
    ...
    struct QueueDeleter {
        ...
        void operator()(guarded_fifo_type *queue_list) noexcept {
            for (guarded_fifo_type *s = queue_list;
                 s != queue_list + mf.num_queues_; ++s) {
                queue_alloc_traits::destroy(mf.alloc_, s);
            }
            queue_alloc_traits::deallocate(mf.alloc_, queue_list,
                                           mf.num_queues_);
        }
    };
   ...
   private:
    ...
    [[no_unique_address]] alignas(2 * L1_CACHE_LINESIZE)
    ...
    void abort_on_data_misalignment() {
        for (guarded_fifo_type *s = queue_list_.get();
            ...
            if (reinterpret_cast<std::uintptr_t>(s) % (2 * L1_CACHE_LINESIZE) !=
                0) {
                std::abort();
            }
        }
   ...
   public:
    ...
    explicit Multififo(size_type /* initial_capacity */,
                       unsigned int num_threads, param_type const &params,
                       allocator_type const &alloc = allocator_type())
        : queue_list_(nullptr, QueueDeleter(*this)),
          num_queues_{num_threads * params.c},
          alloc_{alloc},
    ...
    Handle get_handle() noexcept {
        unsigned int index =
            handle_index_.fetch_add(1, std::memory_order_relaxed);
