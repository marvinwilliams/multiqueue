...
read.histogramtables <- function(file.names, ...) {
  ldply(file.names, function(fn) data.frame(mode=gsub(".txt", "", gsub("_", " C: ", gsub(".*/", "Peek: ", fn))), readhistogram(fn, ...)))
}
...
rtplot <- ggplot(runtimes, aes(x=x, y=y/1000000, group=pq, color=pq)) + geom_line() + geom_point() + labs(x = "p", y = "MOps/s", title = "Operations per second, Peek = 2, C = 4")
pdf("plots.pdf")
print(rplot)
print(dplot)
print(rtplot)
