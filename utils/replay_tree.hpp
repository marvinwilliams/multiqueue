...
struct btree_default_traits {
  ...
  static constexpr int leaf_slots = 8 > (256 / (sizeof(Value)))
                                        ? 8
                                        : (256 / (sizeof(Value)));

  //! Number of slots in each inner node of the tree. Estimated so that each
  //! node has a size of about 256 bytes.
  static const int inner_slots = 8 > (256 / (sizeof(Key) + sizeof(void*)))
                                     ? 8
...
struct node {
  ...
  inline bool is_leafnode() const noexcept { return (level == 0); }
};
...
struct inner_node : public node {
  ...
  inline key_type const& key(size_t s) const { return slotkey[s]; }

  //! True if the node's slots are full.
  inline bool is_full() const noexcept { return (node::slotuse == SlotMax); }

  //! True if few used entries, less than half full.
  inline bool is_few() const noexcept { return (node::slotuse <= SlotMin); }
...
template <typename Key, typename Value, typename KeyOfValue,
          std::size_t SlotMin, std::size_t SlotMax>
...
class ReplayTree {
 ...
 public:
  ...
  using self_type = ReplayTree<key_type, value_type, key_of_value, key_compare,
 ...
 private:
  ...
  using LeafNode =
      leaf_node<key_type, value_type, KeyOfValue, leaf_slotmin, leaf_slotmax>;
 ...
 private:
  ...
  using inner_node_alloc_traits =
      typename alloc_traits::template rebind_traits<InnerNode>;
  using inner_node_alloc_type =
      typename inner_node_alloc_traits::allocator_type;
  using leaf_node_alloc_traits =
      typename alloc_traits::template rebind_traits<LeafNode>;
 ...
 public:
  ...
  //! comparison object.
  explicit ReplayTree(const key_compare& kcf,
  ...
  ReplayTree(InputIterator first, InputIterator last,
             const allocator_type& alloc = allocator_type())
      : root_(nullptr),
        head_leaf_(nullptr),
        tail_leaf_(nullptr),
 ...
 public:
  ...
  class value_compare {
   protected:
    ...
    friend class ReplayTree<key_type, value_type, key_of_value, key_compare,
                            traits, allocator_type>;
 ...
 public:
  ...
  inline iterator begin() { return iterator(head_leaf_, 0); }
  ...
  inline const_iterator begin() const { return const_iterator(head_leaf_, 0); }
 ...
 private:
  ...
  unsigned short find_lower(node_type const* n, key_type const& key) const {
    if constexpr (sizeof(*n) > traits::binsearch_threshold) {
      ...
      TLX_BTREE_PRINT("ReplayTree::find_lower: on "
                      << n << " key " << key << " -> " << lo << " / " << hi);
      ...
      if (self_verify) {
        ...
        TLX_BTREE_PRINT("ReplayTree::find_lower: testfind: " << i);
  ...
  unsigned short find_upper(const node_type* n, const key_type& key) const {
    if constexpr (sizeof(*n) > traits::binsearch_threshold) {
      ...
      TLX_BTREE_PRINT("ReplayTree::find_upper: on "
                      << n << " key " << key << " -> " << lo << " / " << hi);
      ...
      if (self_verify) {
        ...
        TLX_BTREE_PRINT("ReplayTree::find_upper testfind: " << i);
 ...
 public:
  ...
  inline size_type size() const noexcept { return stats_.size; }

  //! Returns true if there is at least one key/data pair in the tree
  inline bool empty() const noexcept { return (size() == size_type(0)); }
  ...
  inline tree_stats const& get_stats() const { return stats_; }
 ...
 public:
  ...
  std::size_t get_rank(key_type const& key) {
    node* n = root_;
    if (!n) return 0;
    std::size_t rank = 0;
 ...
 private:
  ...
  std::pair<iterator, bool> insert_start(key_type const& key,
   ...
   */
  std::pair<iterator, bool> insert_descend(node* n, key_type const& key,
                                           value_type const& value,
                                           std::int64_t delay,
                                           key_type* splitkey,
                                           node** splitnode) {
    if (!n->is_leafnode()) {
      ...
      TLX_BTREE_PRINT("ReplayTree::insert_descend into "
                      << inner->childid[slot]);

      std::pair<iterator, bool> r =
          insert_descend(inner->childid[slot], key, value, delay + inner->delay,
                         &newkey, &newchild);

      if (newchild) {
        TLX_BTREE_PRINT("ReplayTree::insert_descend newchild"
        ...
        if (inner->is_full()) {
          ...
          TLX_BTREE_PRINT("ReplayTree::insert_descend done split_inner:"
          ...
          TLX_BTREE_PRINT("ReplayTree::insert_descend switch: "
          ...
          if (slot == inner->slotuse + 1 &&
            ...
            *splitkey = newkey;
            ...
            TLX_BTREE_PRINT(
                "ReplayTree::insert_descend switching to "
                "splitted node "
                << inner << " slot " << slot);
          }
        }
    ...
    } else  // n->is_leafnode() == true
    {
      ...
      std::copy_backward(leaf->delays + slot, leaf->delays + leaf->slotuse,
                         leaf->delays + leaf->slotuse + 1);
  ...
  void split_leaf_node(LeafNode* leaf, key_type* out_newkey,
    ...
    TLX_BTREE_PRINT("ReplayTree::split_leaf_node on " << leaf);
    ...
    std::copy(leaf->slotdata + mid, leaf->slotdata + leaf->slotuse,
              newleaf->slotdata);
    std::copy(leaf->delays + mid, leaf->delays + leaf->slotuse,
              newleaf->delays);
  ...
  void split_inner_node(InnerNode* inner, key_type* out_newkey,
    ...
    TLX_BTREE_PRINT("ReplayTree::split_inner: mid " << mid << " addslot "
                                                    << addslot);
    ...
    TLX_BTREE_PRINT("ReplayTree::split_inner_node on "
 ...
 public:
  ...
  std::pair<bool, std::int64_t> erase(iterator iter) {
    /* TLX_BTREE_PRINT("ReplayTree::erase_iter(" << iter.curr_leaf << "," */
    /*                                           << iter.curr_slot */
    /*                                           << ") on btree size " <<
     * size()); */
    ...
    if (!root_) return {false, 0};

    std::int64_t delay = 0;

    auto key = iter.key();
    result_t result = erase_iter_descend(iter, root_, delay, nullptr, nullptr,
 ...
 private:
   ...
   */
  result_t erase_iter_descend(iterator const& iter, node* curr,
                              std::int64_t& delay, node* left, node* right,
    ...
    if (curr->is_leafnode()) {
      ...
      auto inner_delay = delay;
      delay += leaf->delays[slot];

      std::copy(leaf->slotdata + slot + 1, leaf->slotdata + leaf->slotuse,
                leaf->slotdata + slot);
      std::copy(leaf->delays + slot + 1, leaf->delays + leaf->slotuse,
                leaf->delays + slot);
      ...
      if (leaf->is_underflow() && !(leaf == root_ && leaf->slotuse >= 1)) {
        ...
        // merger with our parent
          ...
          if (left_parent == parent)
            myres |= merge_leaves(left_leaf, leaf, left_parent, left_delay,
                                  inner_delay);
          else
            myres |= merge_leaves(leaf, right_leaf, right_parent, inner_delay,
                                  right_delay);
        }
    ...
    } else  // !curr->is_leafnode()
    {
      ...
      while (slot <= inner->slotuse) {
        ...
        std::int64_t old_delay = delay;
        delay += inner->delay;
        result = erase_iter_descend(iter, inner->childid[slot], delay, myleft,
                                    myright, myleft_parent, myright_parent,
                                    inner, slot, myleft_delay, myright_delay);
  ...
  result_t merge_leaves(LeafNode* left, LeafNode* right, InnerNode* parent,
    ...
    std::copy(right->delays, right->delays + right->slotuse,
              left->delays + left->slotuse);
  ...
  static result_t merge_inner(InnerNode* left, InnerNode* right,
    ...
    for (std::size_t i = 0; i <= right->slotuse; ++i) {
      add_delay(right->childid[i],
                (right->delay + right_delay) - (left->delay + left_delay));
    }
  ...
  static void shift_right_inner(InnerNode* left, InnerNode* right,
    ...
    for (std::size_t i = left->slotuse - shiftnum + 1; i <= shiftnum; ++i) {
      add_delay(left->childid[i],
                (left->delay + left_delay) - (right->delay + right_delay));
    }
 ...
 public:
  ...
  //! Print out the tree structure with keys onto the given ostream. This
 ...
 private:
  ...
  static void print_node(std::ostream& os, const node* node,
    ...
    if (node->is_leafnode()) {
      ...
      auto innernode = static_cast<InnerNode const*>(node);
...
class TreeIterator {
 public:
  ...
  using value_type =
      std::conditional_t<IsConst, typename Node::value_type const,
 ...
 public:
  ...
  inline reference operator*() const { return curr_leaf->slotdata[curr_slot]; }

  //! Dereference the iterator.
  inline pointer operator->() const { return &curr_leaf->slotdata[curr_slot]; }

  //! Key of the current slot.
  inline key_type const& key() const { return curr_leaf->key(curr_slot); }
