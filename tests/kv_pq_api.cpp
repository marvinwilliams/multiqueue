...
using multiqueue::local_nonaddressable::kv_pq;

TEMPLATE_PRODUCT_TEST_CASE("kv_pq basic operations with different types",
                           "[kv_pq][basic]", std::pair,
                           ((int, int), (int, std::string),
                            (std::string, (std::pair<int, float>)))) {
...
TEST_CASE("kv_pq iterators", "[kv_pq][iterator]") {
  ...
  REQUIRE(std::count_if(q.begin(), q.end(), [i](auto const &p) {
            return p.first == i;
          }) == v[static_cast<size_t>(i)]);
}
...
TEST_CASE("kv_pq move-only", "[kv_pq][move-only]") {
  ...
  [[maybe_unused]] auto extracted = q.extract_top();
  q.pop();
