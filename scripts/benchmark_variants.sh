...
#			else
				...
				${quality_bin} -j ${j} -d ${dist}  2> "${log_dir}/quality_${j}_dist_${dist}_stderr.txt" | ${bin_dir}/utils/evaluate_quality -r "${result_dir}/rank_${j}_dist_${dist}.txt" -d "${result_dir}/delay_${j}_dist_${dist}.txt" -t "${result_dir}/top_delay_${j}_dist_${dist}.txt" -n 1000000 2> "${log_dir}/evaluate_quality_${j}_dist_${dist}_stderr.txt"
			#fi
		#done
	else
	...
	if [[ -x ${quality_bin} ]]; then
		...
		j=16
			...
			#	echo Quality benchmark exists, skipping... >&2
				...
				${quality_bin} -j ${j} -n ${prefill} 2> "${log_dir}/quality_${j}_n_${prefill}_stderr.txt" | ${bin_dir}/utils/evaluate_quality -r "${result_dir}/rank_${j}_n_${prefill}.txt" -d "${result_dir}/delay_${j}_n_${prefill}.txt" -t "${result_dir}/top_delay_${j}_n_${prefill}.txt"  -n 1000000 2> "${log_dir}/evaluate_quality_${j}_n_${prefill}_stderr.txt"
			#fi
		#done
	else
		echo Quality binary not executable, skipping >&2
...
#				echo Quality benchmark exists, skipping... >&2
#			else
#				echo "${quality_bin} -j ${j} -t 5000 -s ${s}" >&2
#				${quality_bin} -j ${j} -t 5000 -s ${s} 2> "${log_dir}/quality_${j}_s_${s}_stderr.txt" | ${bin_dir}/utils/evaluate_quality -r "${result_dir}/rank_${j}_s_${s}.txt" -d "${result_dir}/delay_${j}_s_${s}.txt" -t "${result_dir}/top_delay_${j}_s_${s}.txt" 2> "${log_dir}/evaluate_quality_${j}_s_${s}_stderr.txt"
#			fi
#		done
#	else
#		echo Quality binary not executable, skipping >&2
